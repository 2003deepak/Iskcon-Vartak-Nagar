...
export default function Header() {
  ...
  const navLinks = [
    ...
    { label: "Media", path: "media", href: "/#gallery" },
  ...
  return (
    <header className="sticky top-0 w-full z-50 bg-[#102643] backdrop-blur-md border-b border-amber-500/10 shadow-xl transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-2 sm:gap-4">
        ...
        {/* Brand Logo & Title */}
        <Link href="/" onClick={() => setActiveTab("home")} className="flex items-center gap-2.5 sm:gap-3.5 group shrink-0">
          <div className="relative h-12 w-12 sm:h-16 sm:w-16 flex items-center justify-center shrink-0">
            <Image
              ...
              unoptimized
              priority
              className="object-contain w-full h-full brightness-110 contrast-125 transition-transform duration-300 group-hover:scale-105"
            />
          </div>

          {/* Vertical Separator */}
          <div className="h-8 w-[1px] bg-gradient-to-b from-transparent via-amber-500/30 to-transparent hidden md:block" />

          {/* Text Block */}
          <div className="flex flex-col justify-center">
            <div className="flex items-baseline gap-2.5">
              <span className="font-serif text-[15px] xs:text-lg sm:text-xl font-medium tracking-wide text-[#e6ca85] leading-none">
                ISKCON Vartak Nagar
              </span>
            </div>
            <span className="hidden sm:block text-[10px] sm:text-[11px] tracking-wider uppercase text-slate-300/80 font-medium mt-1 leading-tight">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`relative py-2 text-[13.5px] tracking-wide transition-colors flex items-center gap-1 group ${isActive
              ...
              >
                ...
                <span
                  className={`absolute bottom-0 left-0 h-[2px] rounded-full bg-[#dfb260] transition-all duration-300 ${isActive
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Donate Button (Prominent Gold Pill with Heart Icon) */}
          <Link
            href="/support-us#donate"
            className="inline-flex items-center gap-1.5 sm:gap-2 px-3.5 sm:px-5 py-2 rounded-full bg-[#dfb260] hover:bg-[#cca04b] text-[#060e1b] text-xs sm:text-[13px] font-bold tracking-wide shadow-md shadow-black/20 hover:shadow-amber-950/40 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-300"
          >
            <svg
