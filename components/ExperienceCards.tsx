...
export default function ExperienceCards() {
  ...
  return (
    <section className="w-full py-20 lg:py-24 bg-[#0d1e36] text-slate-100 border-b border-white/5 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-7 items-stretch">
          {experiences.map((exp, idx) => (
            <ScrollReveal
              ...
              className="h-full"
            >
              <div className="group h-full p-7 lg:p-8 rounded-2xl lg:rounded-3xl bg-gradient-to-b from-white/[0.06] to-white/[0.02] hover:from-white/[0.09] hover:to-amber-500/[0.04] border border-white/10 hover:border-amber-400/40 transition-all duration-300 shadow-md hover:shadow-2xl hover:shadow-amber-950/30 transform hover:-translate-y-1.5 flex flex-col justify-between">
                <div>
                  {/* Top Row: Icon Medallion & Time Badge */}
                  <div className="flex items-center justify-between mb-6">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-400/20 to-amber-500/5 border border-amber-400/30 text-amber-300 flex items-center justify-center shadow-inner group-hover:scale-105 group-hover:bg-amber-400/25 group-hover:border-amber-400/60 group-hover:text-amber-200 transition-all duration-300">
                      <span className="material-symbols-outlined text-2xl">
                        {exp.icon}
                      </span>
                    </div>

                    <span className="text-[11px] font-medium text-slate-300 bg-white/5 px-3 py-1 rounded-full border border-white/10 group-hover:border-amber-400/20 transition-colors">
                      {exp.timing}
                    </span>
                  </div>

                  {/* Title & Description */}
                  <h3 className="font-serif text-xl font-normal text-white mb-2.5 group-hover:text-[#fde68a] transition-colors">
                    {exp.title}
                  </h3>
                ...
                <div className="pt-4 border-t border-white/10 group-hover:border-amber-400/20 transition-colors flex items-center justify-between">
                  <span className="text-xs font-semibold text-amber-300 group-hover:text-amber-200 inline-flex items-center gap-1.5 transition-colors">
                    <span>{exp.tag}</span>
